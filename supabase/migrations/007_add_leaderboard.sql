...
create table if not exists public.game_leaderboard_players (
...
alter table public.game_leaderboard_players enable row level security;

create policy "Allow public read" on public.game_leaderboard_players
  for select using (true);

-- Add player_id FK to game_results (nullable — anonymous games still allowed)
alter table public.game_results
  add column if not exists player_id uuid references public.game_leaderboard_players(id) on delete set null;
